...
/**
 * A servlet to respond to requests for Molecular Testing Report files for cancer studies.
 * Adapted from PathologyReportView.java
 * 
 * @author Joshua Stretton
 */
public class MolecularTestingReportView extends PdfFileView {
    ...
    public void init() throws ServletException {
        super.init();
        DATA_DIRECTORY = GlobalProperties.getInternalMolecularTestingRoot();
    }
    ...
    boolean validRequest(HttpServletRequest request) throws IOException, DaoException {
        ...
        if (path.length != 3) {
            setError(request, HttpServletResponse.SC_INTERNAL_SERVER_ERROR, "Unable to process requested pathology " +
                "report path. Ensure it is in the format cbioportal/molecular_testing_report/study_id.patient_id.sample_id.pdf");
            return false;
        }
        ...
        if (DATA_DIRECTORY == null) {
            setError(request, HttpServletResponse.SC_NOT_FOUND, "The internal location of molecular testing reports is undefined");
            return false;
        }
        File requestedFile = getRequestedFile(request);
        if (!requestedFile.exists() || requestedFile.isDirectory()) {
            setError(request, HttpServletResponse.SC_NOT_FOUND, "Unable to locate molecular testing report '" 
